[place]
s0_mode_matches = class=ProjectM-pulseaudio | class=mpv;
s0_mode_modes = 1;
s0_viewport_matches = class=Epiphany | class=Firefox;(class=Pantheon-terminal | class=Gnome-control-center | class=Ccsm | class=Pavucontrol | class=Gimp | class=Gedit | class=Dconf-editor | class=Gconf-editor);class=ProjectM-pulseaudio | class=mpv;class=Files |class=Quodlibet;
s0_viewport_x_values = 1;2;3;4;
s0_viewport_y_values = 1;1;1;1;

[freewins]
s0_snap_threshold = 50
s0_circle_color = #54befb80
s0_line_color = #1800ffff
s0_cross_line_color = #1800ffff

[showmouse]
s0_color = #ffdf3fff

[shift]
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300

[cubeaddon]
s0_top_next_key = Disabled
s0_reflection = false
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300
s0_deformation = 2
s0_draw_top = false
s0_draw_bottom = false
s0_top_images = ;
s0_bottom_images = ;

[workspacenames]
s0_names = South;West;North;East;
s0_display_time = 0.500000
s0_text_font_size = 14
s0_back_color = #2f2f4f99

[obs]
s0_opacity_matches = class=Wingpanel | class=Plank | type=dropdownmenu | type=popupmenu | type=tooltip | type=menu | type=notification | type=splash;class=Gnome-control-center | class=Ccsm | class=Pavucontrol |  class=Gedit;class=Files |class=Quodlibet;
s0_opacity_values = 70;80;80;0;
s0_brightness_increase_button = <Alt>Button4
s0_brightness_decrease_button = <Alt>Button5
s0_brightness_values = 0;
s0_saturation_increase_button = <Alt>Button4
s0_saturation_decrease_button = <Alt>Button5

[scaleaddon]
s0_layout_mode = 1
s0_natural_precision = 20.000000

[scale]
s0_initiate_edge = 

[vpswitch]
s0_next_button = Disabled
s0_prev_button = Disabled
s0_initiate_button = Disabled

[mousepoll]
s0_mouse_poll_interval = 500

[animationaddon]
s0_beam_color = #7f7f7fff
s0_fire_color = #ff3305ff

[move]
s0_lazy_positioning = true

[thumbnail]
s0_fade_speed = 0.250000
s0_thumb_color = #2f2f4f7f
s0_mipmap = true
s0_current_viewport = false
s0_title_enabled = false
s0_font_size = 14
s0_font_background_color = #0000007f

[decor]
s0_active_shadow_color = #00000080
s0_inactive_shadow_color = #000000ff
s0_mipmap = true
s0_decoration_match = (any) & !(class=mpv)

[switcher]
s0_next_key = Disabled
s0_prev_key = Disabled
s0_next_all_key = Disabled
s0_prev_all_key = Disabled
s0_next_no_popup_key = <Alt>Tab
s0_prev_no_popup_key = <Shift><Alt>Tab
s0_focus_on_switch = true
s0_background_color = #333333d9

[expo]
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300

[grid]
s0_put_restore_key = Disabled
s0_top_left_corner_action = 7
s0_top_edge_action = 8
s0_top_right_corner_action = 9
s0_bottom_left_corner_action = 1
s0_bottom_edge_action = 2
s0_bottom_right_corner_action = 3
s0_left_edge_threshold = 45
s0_right_edge_threshold = 45
s0_top_edge_threshold = 45
s0_bottom_edge_threshold = 45
s0_outline_color = #ffffffff
s0_fill_color = #2f2f4f4f

[resize]
s0_initiate_button = <Alt>Button3
s0_mode = 2
s0_border_color = #ffffffff

[staticswitcher]
s0_next_key = Disabled
s0_prev_key = Disabled
s0_next_all_key = <Alt>Tab
s0_prev_all_key = <Shift><Alt>Tab
s0_brightness = 10
s0_opacity = 40
s0_icon = false
s0_icon_only = true
s0_focus_on_switch = true
s0_bring_to_front = false
s0_highlight_mode = 2
s0_highlight_rect_hidden = 2
s0_highlight_color = #2f2f4f00
s0_highlight_border_color = #ffffffff
s0_highlight_border_inlay_color = #2f2f4f00
s0_use_background_color = true
s0_background_color = #33333300

[snap]
s0_snap_type = 0;1;
s0_edges_categories = 0;1;
s0_attraction_distance = 10

[screenshot]
s0_selection_outline_color = #2f2f4f9f
s0_selection_fill_color = #2f2f4f4f

[core]
s0_active_plugins = core;composite;opengl;compiztoolbox;obs;imgjpeg;decor;grid;resize;staticswitcher;snap;regex;resizeinfo;workarounds;imgpng;gnomecompat;wallpaper;neg;commands;cube;text;firepaint;place;workspacenames;scale;titleinfo;mousepoll;move;thumbnail;rotate;scalefilter;cubeaddon;scaleaddon;
s0_audible_bell = false
s0_outputs = 1920x1080+0+0;
s0_autoraise_delay = 500
s0_minimize_window_key = <Super>h
s0_maximize_window_key = <Super>Up
s0_unmaximize_window_key = Disabled
s0_unmaximize_or_minimize_window_key = <Super>Down
s0_window_menu_button = <Alt>Button2
s0_show_desktop_key = Disabled
s0_toggle_window_maximized_key = <Alt>F10
s0_toggle_window_shaded_key = Disabled

[opengl]
s0_texture_filter = 2
s0_texture_compression = true

[resizeinfo]
s0_fade_time = 250
s0_always_show = true
s0_resizeinfo_font_size = 14
s0_text_color = #ffffffff
s0_gradient_1 = #2f2f4f4f
s0_gradient_2 = #2f2f4f4f
s0_gradient_3 = #2f2f4f4f
s0_outline_color = #ffffffff

[workarounds]
s0_keep_minimized_windows = true
s0_ooo_menu_fix = false
s0_java_fix = false
s0_java_taskbar_fix = false
s0_aiglx_fragment_fix = false
s0_initial_damage_complete_redraw = false

[rotate]
s0_flip_time = 250
s0_speed = 4.000000
s0_initiate_button = <Alt><Super>Button1
s0_rotate_left_button = Button8
s0_rotate_right_button = Button9

[gnomecompat]
s0_main_menu_key = <Super>space
s0_run_key = Disabled
s0_command_window_screenshot = gnome-screenshot -w
s0_run_command_window_screenshot_key = <Super>Print
s0_command_terminal = pantheon-terminal
s0_run_command_terminal_key = <Control><Alt>t

[ezoom]
s0_zoom_box_outline_color = #2f2f4f9f
s0_zoom_box_fill_color = #2f2f2f4f

[wallpaper]
s0_bg_image = ;
s0_bg_image_pos = 0;
s0_bg_fill_type = 0;
s0_bg_color1 = #00000000;
s0_bg_color2 = #00000000;

[animationplus]
s0_bonanza_color = #ff3305ff

[cube]
s0_top_color = #ffffff00
s0_bottom_color = #ffffff00
s0_skydome_image = /home/zombie/Pictures/wallpaper/skydomes/8994556164_cdf9716a6a_o-retouch.png
s0_skydome_animated = true
s0_skydome_gradient_start_color = #00000000
s0_skydome_gradient_end_color = #00000000
s0_active_opacity = 0.000000
s0_inactive_opacity = 0.000000
s0_transparent_manual_only = false

[composite]
s0_unredirect_match = (any) & !(class=Audience) & !(class=Totem) & !(class=MPlayer) & !(class=Vlc) & !(class=Plugin-container) & !(class=Firefox)

[wall]
s0_outline_color = #333333d9
s0_background_gradient_base_color = #cccce6d9
s0_background_gradient_highlight_color = #f3f3ffd9
s0_background_gradient_shadow_color = #f3f3ffd9
s0_thumb_gradient_base_color = #33333359
s0_thumb_gradient_highlight_color = #3f3f3f3f
s0_thumb_highlight_gradient_base_color = #fffffff3
s0_thumb_highlight_gradient_shadow_color = #dfdfdfa6
s0_arrow_base_color = #e6e6e6d9
s0_arrow_shadow_color = #dcdcdcd9
s0_up_key = <Super>Prior
s0_down_key = <Super>Next
s0_up_window_key = <Shift><Super>Prior
s0_down_window_key = <Shift><Super>Next

[put]
s0_put_viewport_left_key = <Shift><Alt>Left
s0_put_viewport_right_key = <Shift><Alt>Right
s0_put_left_key = Disabled
s0_put_right_key = Disabled
s0_put_top_key = Disabled
s0_put_bottom_key = Disabled
s0_put_topleft_key = Disabled
s0_put_topright_key = Disabled
s0_put_bottomleft_key = Disabled
s0_put_bottomright_key = Disabled

[firepaint]
s0_initiate_button = <Super>Button1
s0_clear_key = Disabled
s0_clear_button = <Super>Button3
s0_fire_color = #ff3305ff

